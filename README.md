...
The heart of nnoq is a single operator, `:=`, which is governed by the following axioms:  
    ...
    4. ${\displaystyle {\Gamma \vdash A := B \qquad \qquad \Pi \vdash f(A, C): T \over \Gamma \vdash f(A, C) := f(B, C)}}$ (congruence1)    
...
the following five axioms are for the type analysis (inference and checking):  
    ...
    11. ${\displaystyle {\Pi, x: T_x \vdash A: T_A \qquad \qquad \Pi, A[x/Y]: T_A \vdash Y: T_x \over \Pi \vdash A[x/Y] : T_A}}$    
